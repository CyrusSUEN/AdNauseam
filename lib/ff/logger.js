...
var Logger = require('sdk/core/heritage').Class({
  ...
  reset: function() {
    ...
    try {
      ...
      if (!this.logFile.exists()) { // check if .txt log exists
        ...
        console.log("log size: ", this.logFile.fileSize);
  ...
  truncateLogFile: function () {
    ...
    // while (this.logFile.fileSize > 8192000) {
    if (true) {
      ...
      istream.init(this.logFile, -1, 0, 0);
      cstream.init(istream, "UTF-8", 0, 0);

      var str = {};
      var read = 0;
      var data;
      var pos = istream.tellp();
      console.log("pos: ", pos);
      do {
        read = cstream.readString(0xffffffff, str); // read and put it in str.value
        data += str.value;
      } while (read != 0);
      cstream.close();
